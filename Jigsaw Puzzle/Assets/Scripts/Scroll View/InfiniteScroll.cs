...
namespace Mopsicus.InfiniteScroll {
	...
	public class InfiniteScroll : MonoBehaviour, IDropHandler {
        ...
        private void OnEnable()
        {
			...
			{
				Prefab.transform.GetChild(i).GetComponent<RectTransform>().sizeDelta = new Vector2(GameConfig.VERTICAL_PUZZLE_SIZE * CommonUltilities.GetRescaleRatio()
					, GameConfig.VERTICAL_PUZZLE_SIZE * CommonUltilities.GetRescaleRatio());
			}
		}
		...
		void InitVertical (int count) {
			float height = CalcSizesPositions (count) + offSetHeight + (200 * CommonUltilities.GetRescaleRatio());
			CreateViews ();
			_previousPosition = 0;
			_count = count;
		...
		float CalcSizesPositionsVertical (int count) {
			...
			for (int i = 0; i < count; i++) {
				...
				_positions[i] = -(TopPadding + i * ItemSpacing + result + offSetHeight);
				result += _heights[i];
			}
